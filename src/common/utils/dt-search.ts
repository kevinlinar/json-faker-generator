...
import { DTDataRequestColumn } from '@shared/interfaces/data-tables';

export const dtSearch = <T>(rows: T[], searchTerm: string, columns: DTDataRequestColumn[]): T[] => {
	...
	const search = rows.filter((item) => {
		const concatenatedValues = columns
			...
			.join(' ');
		return lowerCaseSearchTerms.some((term) => concatenatedValues.includes(term));
	});
	return search;
};
